...
> ⚠️ **Demo note:** This is a portfolio showcase, not a production service. Calls are capped at **2 minutes** (enforced both client-side and via Agora token expiry), and **all data accounts and meetings is automatically wiped every 12 hours** via a scheduled cleanup job. Don't store anything you need to keep. See [Demo limitations](#demo-limitations) below.
...
This project is deployed publicly to showcase working, real-world skills not as a service you should rely on for anything persistent. Two intentional constraints:

**Calls are capped at 2 minutes.** Unrestricted video calls on a public demo could be used by anyone to run indefinite Agora sessions against my API credentials. The cap is enforced twice: client-side (a visible countdown that ends the call cleanly) and server-side (the Agora token itself expires shortly after, so the limit holds even if someone bypasses the UI). The underlying code supports arbitrary-length calls see `CALL_LIMIT_SECONDS` in `components/VideoRoom.jsx` and `expireSeconds` in `lib/agora.js` the 2-minute value is just this deployment's configuration.

**All data resets every 12 hours.** A scheduled job (`/api/cron/cleanup`, triggered by an external cron service since Vercel's free tier caps built-in cron at once/day) wipes every account and meeting from the database on a fixed interval. This keeps a public demo's free-tier database from filling up indefinitely and avoids accumulating stale test data. **Accounts and meetings created on the live demo will not persist** — don't use it for anything you need to keep. Running your own instance locally (see Getting Started above) has no such limit.
