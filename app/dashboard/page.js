...
export default function DashboardPage() {
  ...
  const upcoming = meetings
    .filter((m) => new Date(m.endTime) > new Date() && m.status !== 'cancelled')
  ...
  const inputStyle = { border: '1px solid var(--line)', '--tw-ring-color': 'var(--indigo)' };
    ...
    <main className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-6">
          <div className="rounded-2xl p-6 bg-white" style={cardStyle}>
              ...
              >
                {upcoming.map((m) => (
                  <li
                    key={m._id}
                    href={`/meetings/${m._id}`}
                    className="block rounded-xl px-3.5 py-2.5 transition-colors hover:bg-gray-50"
